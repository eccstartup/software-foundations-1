...
Theorem iff_refl : forall P : Prop,
  P <-> P.
Proof.
  intros. split; intros; apply H.  Qed.
...
Proof.
  intros. split. intros.
    inversion H as [HP HQ].
      inversion H0 as [H0Q H0R].
      apply H0Q. apply HP. apply H1.
    inversion H as [HP HQ].
      inversion H0 as [H0Q H0R].
      intros. apply HQ. apply H0R. apply H1.  Qed.
...
Definition truth (P Q R : Prop) (ip : (P \/ Q) /\ (P \/ R)) : P \/ (Q /\ R) :=
  match ip with
    | conj pq pr => match pq with
      | or_introl p => or_introl P (Q /\ R) p
      | or_intror q => match pr with
        | or_introl p => or_introl P (Q /\ R) p
        | or_intror r => or_intror P (Q /\ R) (conj Q R q r)
      end
    end
  end.

(** **** Exercise: 2 stars (or_distributes_over_and_2) *)
Theorem or_distributes_over_and_2_alt : forall P Q R : Prop,
  (P \/ Q) /\ (P \/ R) -> P \/ (Q /\ R).
Proof.
